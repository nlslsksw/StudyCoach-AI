...
enum DemoData {
    /// Screenshots are taken in German and English – the sample data follows the app language.
    static var english: Bool { Locale.current.language.languageCode?.identifier != "de" }
    static func t(_ de: String, _ en: String) -> String { english ? en : de }
    /// Maps the German demo subject names to their English counterparts.
    static func subjectName(_ de: String) -> String {
        guard english else { return de }
        return ["Mathe": "Maths", "Deutsch": "English", "Englisch": "French", "Physik": "Physics",
                "Geschichte": "History", "Bio": "Biology", "Sachkunde": "Science", "Sport": "PE",
                "Musik": "Music", "Kunst": "Art", "Ethik": "Ethics"][de] ?? de
    }
    ...
    static func seedParent(store: DataStore) {
        ...
        lena.subjects = [
            Subject(name: t("Deutsch", "English"), icon: "text.book.closed.fill", colorName: "orange"),
            Subject(name: t("Mathe", "Maths"), icon: "function", colorName: "blue"),
            Subject(name: t("Sachkunde", "Science"), icon: "leaf.fill", colorName: "green")
        ]
        lena.grades = [
            Grade(subject: subjectName("Deutsch"), grade: 2, date: day(-6), type: .schriftlich),
            Grade(subject: subjectName("Mathe"), grade: 1, date: day(-13), type: .schriftlich)
        ]
        lena.sessions = [
        ...
        lena.entries = [CalendarEntry(title: t("Deutsch Diktat", "English dictation"), date: day(6, hour: 8), type: .klassenarbeit)]
    ...
    private static func makeSchoolYear() -> (SchoolYear, [Subject]) {
        ...
        let subjects = [
            Subject(name: t("Mathe", "Maths"), icon: "function", colorName: "blue", schoolYearId: year.id, targetGrade: 2.0),
            Subject(name: t("Deutsch", "English"), icon: "text.book.closed.fill", colorName: "orange", schoolYearId: year.id, targetGrade: 2.5),
            Subject(name: t("Englisch", "French"), icon: "globe", colorName: "green", schoolYearId: year.id, targetGrade: 2.0),
            Subject(name: t("Physik", "Physics"), icon: "atom", colorName: "purple", schoolYearId: year.id),
            Subject(name: t("Geschichte", "History"), icon: "building.columns.fill", colorName: "red", schoolYearId: year.id),
            Subject(name: t("Bio", "Biology"), icon: "leaf.fill", colorName: "mint", schoolYearId: year.id, targetGrade: 3.0)
        ]
    ...
    private static func makeGrades() -> [Grade] {
        [
            Grade(subject: subjectName("Mathe"), grade: 2, date: day(-2), type: .schriftlich),
            Grade(subject: subjectName("Mathe"), grade: 3, date: day(-16), type: .muendlich),
            Grade(subject: subjectName("Deutsch"), grade: 2, date: day(-5), type: .schriftlich),
            Grade(subject: subjectName("Deutsch"), grade: 3, date: day(-19), type: .muendlich),
            Grade(subject: subjectName("Englisch"), grade: 1, date: day(-8), type: .schriftlich),
            Grade(subject: subjectName("Englisch"), grade: 2, date: day(-14), type: .muendlich),
            Grade(subject: subjectName("Physik"), grade: 3, date: day(-10), type: .schriftlich),
            Grade(subject: subjectName("Geschichte"), grade: 2, date: day(-12), type: .muendlich),
            Grade(subject: subjectName("Bio"), grade: 4, date: day(-4), type: .schriftlich)
        ]
    }
    ...
    private static func makeSessions() -> [StudySession] {
        ...
        return plan.map { StudySession(subject: subjectName($0.1), date: day($0.0, hour: 15 + abs($0.0) % 3), minutes: $0.2) }
    }

    private static func makeEntries() -> [CalendarEntry] {
        [
            CalendarEntry(title: t("Mathe Klassenarbeit", "Maths exam"), date: day(3, hour: 8), type: .klassenarbeit, notes: t("Lineare Funktionen, Kapitel 3–4", "Linear functions, chapters 3–4")),
            CalendarEntry(title: t("Englisch Vokabeltest", "French vocab test"), date: day(9, hour: 10), type: .klassenarbeit, notes: t("Unit 2", "Unit 2")),
            CalendarEntry(title: t("Physik Test", "Physics test"), date: day(16, hour: 9), type: .klassenarbeit),
            CalendarEntry(title: t("Bio Referat", "Biology presentation"), date: day(5, hour: 11), type: .erinnerung),
            CalendarEntry(title: t("Lerntag Mathe", "Maths study day"), date: day(1, hour: 16), type: .lerntag)
        ]
    }
        ...
        for (dayIdx, lessons) in week.enumerated() {
            for (i, lesson) in lessons.enumerated() {
                slots.append(TimetableSlot(weekday: dayIdx + 1, lesson: i + 1, startTime: times[i].0, endTime: times[i].1,
                                           subject: subjectName(lesson.0), room: lesson.1))
            }
        }
        return slots
    }
    ...
    private static func makeHomework() -> [Homework] {
        [
            Homework(subject: subjectName("Mathe"), title: t("S. 87 Nr. 3–7", "p. 87 no. 3–7"), dueDate: day(1, hour: 8)),
            Homework(subject: subjectName("Deutsch"), title: t("Charakterisierung Faust", "Macbeth character study"), dueDate: day(2, hour: 8)),
            Homework(subject: subjectName("Englisch"), title: t("Vocabulary Unit 2", "Vocabulary unit 2"), dueDate: day(1, hour: 8), isDone: true),
            Homework(subject: subjectName("Bio"), title: t("Referat vorbereiten", "Prepare presentation"), dueDate: day(4, hour: 8))
        ]
    }
}
#endif
